...
### Pending CI follow-ups (once conda-forge publishes ANUGA 3.3.7 with osx-arm64)
1. Bump `environment.yml` `anuga=3.3.6` → `3.3.7` (and drop the "bump once conda-forge publishes" comment).
2. Add a **`macos-latest` (arm64) lane to `conda-env.yml`** to natively run the test suite against the cross-compiled conda osx-arm64 package — the feedstock build cross-compiles arm64 and *skips its tests*, so this is the native runtime check that closes that gap (the build stays a cheap cross-compile; only the test runs on arm64 hardware).
3. Drop the Windows `continue-on-error` in `pip.yml` once meshpy ships a self-contained Windows wheel (`inducer/meshpy#150` / PR #151).
