...
for t in domain.evolve(yieldstep=dt, outputstep=out_dt, finaltime=ft):
    ...
    # Calculate the coupling flux (smoothed + clamped to available water),
    # step SWMM by dt, and feed the realised SWMM flow back to ANUGA. The
    # Coupler sets inlet1/outlet to the realised flux; the outlet is then
    # overridden below to also return water leaving the system at the outfall.
