"""Simple channel + culvert: ANUGA <-> SWMM coupling, hand-built.

This is the canonical *hand-built* coupling example: it sets up the ANUGA inlet
operators and the pyswmm simulation explicitly, then drives the per-step
exchange with ``anuga_drainage.Coupler``. Compare with ``run_from_inp.py``,
which builds the same model in one ``couple_from_inp`` call from the ``.inp``;
read this one to see what that call does under the hood.

Water that reaches the SWMM outfall is returned to the 2D surface at the outlet
inlet (the outfall-return policy), so the whole system conserves except for
SWMM's own finite-difference loss (R_pipe ~0.08 m^3; see the VolumeBalance
summary printed at the end).

    python run_swmm_short.py
"""
import anuga
import numpy as np
...
dt = 1.0         # yield/coupling step. Whole seconds only: stock pyswmm 2.1.0
                 # strides in integer seconds (SWMM still routes internally at
                 # the smaller ROUTING_STEP in the .inp; this is just the
                 # surface<->pipe exchange interval).
...
domain.set_name('domain_swmm_short')
...
# Channel-spanning inlet/outlet footprints (width cw), so flow is captured into
# the culvert rather than overtopping the embankment. weir_length ~ polygon
# perimeter, manhole_area ~ polygon area.
cw = 8
inlet_region = anuga.Region(domain, polygon=[[20, 10 - cw / 2], [22, 10 - cw / 2],
                                             [22, 10 + cw / 2], [20, 10 + cw / 2]])
...
# The Coupler owns the read-depths -> calculate_Q -> smooth -> clamp -> step
# SWMM -> feed realised flow back sequence (see anuga_drainage.Coupler). Inlets
# are ordered to match the backend's junction heads.
...
# Localises the mass-balance loss into the ANUGA budget, the pipe budget, and
# the surface<->pipe handoff. outfall_inlet=1: the outlet inlet receives the
# returned outfall water (an internal transfer, not a system sink).
